...
while(True):
    ...
    isIdle = True
    for i in range(len(machineStates)):
        isIdle = isIdle and len(machineStates[i]['inputs']) == 0

        if not len(machineStates[i]['inputs']) == 0:
            machineStates[i]['inputs'].append(-1)
        runProgram(machineStates, i, -1)
        outputs = machineStates[i]['outputs']

        isIdle = isIdle and len(outputs) == 0
        ...
        while(len(outputs) != 0):
            ...
            if(addr == 255):
                ...
                machineStates[addr]['inputs'].extend([x, y])
                isIdle = False

    if isIdle:
        if(len(lastNatDelv) > 0 and lastNat[1] == lastNatDelv[1]):
            print(f"WINNER: Machine 0 recving lastNat=[{lastNat[0]}, {lastNat[1]}], but lastNatDelv=[{lastNatDelv[0]}, {lastNatDelv[1]}]")
            exit()
        machineStates[0]['inputs'] = [lastNat[0], lastNat[1]]
        lastNatDelv = [lastNat[0], lastNat[1]]
