...
for i in range(len(outputs)):
    if(outputs[i] == 10):
        if(i == len(outputs)-1 or outputs[i+1] == 10): 
            break
        rowIdx += 1
        shipMap.append([])
        continue
    shipMap[rowIdx].append(chr(outputs[i]))

accum = 0
robotChars = ['^', 'v', '<', '>']
for rowIdx in range(len(shipMap)):
    for colIdx in range(len(shipMap[rowIdx])):
        mapChar = shipMap[rowIdx][colIdx]
        if(mapChar == "." or mapChar in robotChars):
            print(mapChar, end="")
            continue
        if(rowIdx == 0 or rowIdx == (len(shipMap) - 1) or colIdx == 0 or colIdx == (len(shipMap[rowIdx]) - 1)):
            print(mapChar, end="")
            continue
        above = shipMap[rowIdx-1][colIdx]
        below = shipMap[rowIdx+1][colIdx]
        left = shipMap[rowIdx][colIdx-1]
        right = shipMap[rowIdx][colIdx+1]
