...
for y in range(len(beamMap) - SHIP_H):
    for x in range(len(beamMap[0]) - SHIP_W):
        if beamMap[y][x] == '.': continue

        isBeam = beamMap[y][x] != '.'
        hasWidth = beamMap[y][x + SHIP_W - 1] != '.'
        hasHeight = beamMap[y + SHIP_H - 1][x] != '.'
        # print(f"  [{x:2}, {y:2}]: isBeam={isBeam} ({beamMap[y][x]}), hasWidth={hasWidth} ({beamMap[y][x + SHIP_W - 1]}), hasHeight={hasHeight} ({beamMap[y + SHIP_H - 1][x]})")
        if isBeam and hasWidth and hasHeight:
            print(f"Found ship corner: [{x}, {y}] (code={x*10000 + y})")
            exit(0)



## BELOW was used to generate the map. Took about 5 mins for 2k x 2k
...
# MAX_X = 2000
# MAX_Y = 2000
# print(f"Creating grid [{MAX_X}, {MAX_Y}]...")
# count = 0
# accumBytes = 0
# firstHit_x = 0
# fp = open("output_1.txt", "w")
# t0 = time.perf_counter()
# for y in range(0, MAX_Y):
#     accumStr = '.' * firstHit_x
#     hasFirstHit = False
#     for x in range(firstHit_x, MAX_X):
#         states = [ { "isFinished": False, "relativeBase": 0, "codes": codes.copy(), "instPtr": 0, 
#                 "inputs": [x, y], "outputs": [] } ]
#         runProgram(states, 0, -1)
#         isHit = states[0]['outputs'][0] == 1
#         if(isHit):
#             if(not hasFirstHit and firstHit_x < x): firstHit_x = x
#             hasFirstHit = True
#             accumStr += "#"
#             count += 1
#         elif not hasFirstHit:
#             accumStr += "."
#         elif hasFirstHit:
#             accumStr += '.' * (MAX_X-x)
#             break
#     accumBytes += fp.write(accumStr + "\n")
#     if(y % 100 == 0):
#         t1 = time.perf_counter()
#         print(f"    ...wrote line {y:04d} of {MAX_Y} ({round(100*y/MAX_Y)}%); accumBytes={accumBytes} accumTime={t1-t0:0.3f} sec")
# t1 = time.perf_counter()

# print(f"Wrote {accumBytes} bytes to file; Total hits: {count}; elapsed={t1-t0:0.3f} sec")


# # 
# # [865, 1001] 8651001 is too low
# # [828, 1005] 8281005 is too low
